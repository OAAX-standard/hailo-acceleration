 ...
 *   runtime_init() -> runtime_load_models() ->
 *   { runtime_enqueue_input() / runtime_retrieve_output() } ->
 *   runtime_cleanup()
 *
 * On any non-SUCCESS return, call runtime_get_error() for a human-readable
 * description.
 */

/* Initialize the runtime with the given configuration.
 * Must be called before any other function.
 * Config keys: "log_level" (int 0-3), "log_file" (path). */
 ...
 * The runtime takes ownership of input_tensors and will free it after
 * inference. */
 ...
 * Returns RUNTIME_STATUS_NO_OUTPUT_AVAILABLE if no result is ready within the
 * timeout. The caller is responsible for freeing *output_tensors. */
...
/* Return a JSON string with runtime diagnostics: loaded_models,
 * requests_in_flight, backend_version. Returns NULL if the runtime is not
 * initialized. */
