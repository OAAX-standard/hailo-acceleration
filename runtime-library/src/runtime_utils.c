...
char **get_input_names(OrtSession *session, OrtAllocator *allocator, int *count) {
    ...
    *count = (int)n;
    ...
    for (int i = 0; i < (int)n; i++) {
        process_ort_status(api->SessionGetInputName(session, i, allocator, &tmp[i]));
...
char **get_output_names(OrtSession *session, OrtAllocator *allocator, int *count) {
    ...
    *count = (int)n;
    ...
    for (int i = 0; i < (int)n; i++) {
        process_ort_status(api->SessionGetOutputName(session, i, allocator, &tmp[i]));
        names[i] = (char *)malloc(strlen(tmp[i]) + 1);
        strcpy(names[i], tmp[i]);
        process_ort_status(api->AllocatorFree(allocator, tmp[i]));
    }
...
ONNXTensorElementDataType tensor_element_type_to_ort_type(TensorElementType type) {
    switch (type) {
        ...
        case DATA_TYPE_BFLOAT16: return ONNX_TENSOR_ELEMENT_DATA_TYPE_BFLOAT16;
...
TensorElementType ort_type_to_tensor_element_type(ONNXTensorElementDataType ort_type) {
    switch (ort_type) {
        ...
        case ONNX_TENSOR_ELEMENT_DATA_TYPE_BFLOAT16: return DATA_TYPE_BFLOAT16;
