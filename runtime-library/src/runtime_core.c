...
static void* worker_loop(void* arg)
{
    ...
    while (1)
    {
        ...
        log_debug(
            logger,
            "[model %d] Replica %d running inference (request id=%d)",
            m->model_id,
            wa->replica_id,
            input->id);
...
static int load_one_model(int idx, const ModelConfig* mc, int threads_per_replica, int n_replicas)
{
    ...
    {
        set_error(
            "[model %d] Failed to append Hailo execution provider — check that libonnxruntime_providers_hailo.so is alongside libRuntimeLibrary.so",
            idx);
        return 1;
    }
    ...
    for (int r = 0; r < n_replicas; r++)
    {
        ...
        else if (mc->model_data && mc->model_size > 0)
        {
            if (r == 0)
                log_info(
                    logger,
                    "[model %d] Loading from memory (%zu bytes, %d replica(s), %d thread(s) each)",
                    idx,
                    mc->model_size,
                    n_replicas,
                    threads_per_replica);
            session_err = process_ort_status(api->CreateSessionFromArray(
                m->env,
                mc->model_data,
                mc->model_size,
                m->session_options,
    ...
    if (process_ort_status(
            api->CreateCpuMemoryInfo(OrtArenaAllocator, OrtMemTypeDefault, &m->memory_info))
        != 0)
    {
    ...
    if (process_ort_status(api->CreateAllocator(m->sessions[0], m->memory_info, &m->allocator))
        != 0)
    {
    ...
    log_info(
        logger,
        "[model %d] Ready (%d inputs, %d outputs, %d replica(s), %d thread(s) each)",
        idx,
        m->num_inputs,
        m->num_outputs,
        n_replicas,
