/**
 * Basic test for the OAAX v2 runtime interface (Hailo backend).
 *
...
int main(int argc, char** argv) {
    std::cout << "=== OAAX v2 Runtime Test (Hailo) ===" << std::endl;
    ...
    ASSERT(strcmp(name, "OAAX Hailo Runtime") == 0, std::string("unexpected name: ") + name);
    ...
    if (argc > 1) {
        ...
        /* Re-init after the failed load above left models_loaded clean. */
        ASSERT(runtime_cleanup() == RUNTIME_STATUS_SUCCESS, "cleanup before reload failed");
        ASSERT(runtime_init(cfg) == RUNTIME_STATUS_SUCCESS, "re-init before reload failed");
