/**
 * Integration benchmark: OAAX v2 runtime with Hailo-accelerated YOLO models.
 *
 ...
 *
 * Defaults: runs=30, warmup=5, batch=1, input-dtype=f32, in-flight=5,
 *           imgsz=640, input-name=images
 */
...
static Tensors* make_input(int batch, int request_id, TensorElementType dtype, int imgsz,
    ...
    if (!ts->tensors) { free(ts); return nullptr; }
...
static bool validate_output(const Tensors* out) {
    /* Basic structural check — output must have at least one non-empty tensor. */
    if (!out || out->num_tensors < 1) return false;
    for (int i = 0; i < out->num_tensors; ++i) {
        if (out->tensors[i].rank <= 0) return false;
        if (out->tensors[i].data_size == 0 || out->tensors[i].data == nullptr) return false;
    }
    return true;
}
 ...
 */
static std::vector<double> run_batch(int n, std::vector<Clock::time_point>& send_times,
                                     bool do_validate, int batch, TensorElementType dtype,
                                     int imgsz, int max_in_flight, const char* input_name) {
    ...
    std::thread consumer([&]() {
        for (int i = 0; i < n; ++i) {
            ...
            if (st != RUNTIME_STATUS_SUCCESS) { ok = false; return; }

            latencies[output->id] = Ms(Clock::now() - send_times[output->id]).count();
            in_flight--;

            if (do_validate && i == 0 && !validate_output(output)) {
                std::cerr << "FAIL: output validation failed on first result" << std::endl;
                ok = false;
            }
            free_tensors(output);
        }
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== OAAX YOLO Benchmark (Hailo) ===" << std::endl;
    ...
    std::cout << "[3] Warming up (" << warmup << " runs)..." << std::endl;
    {
        ...
        CHECK(!run_batch(warmup, ts, false, batch, input_dtype, imgsz, in_flight, input_name).empty(),
              "warmup failed");
    }
    std::cout << "  Done" << std::endl;

    // ── 4. Benchmark ──────────────────────────────────────────────────────────
    std::cout << "[4] Benchmarking (" << runs << " runs, batch=" << batch
              << ", in-flight=" << in_flight << ")..." << std::endl;
    std::vector<Clock::time_point> send_times(runs);
    auto bench_start = Clock::now();
    auto latencies = run_batch(runs, send_times, !no_validate, batch, input_dtype, imgsz,
                               in_flight, input_name);
