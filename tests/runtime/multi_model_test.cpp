 ...
 *  - Output shapes are valid (use --no-validate to skip shape checks for Hailo models)
 *
 * Usage:
 *   ./multi_model_test <model.onnx> [model2.onnx]
 *                      [--input-name NAME] [--imgsz N] [--no-validate]
 *
...
static Tensors* make_input(int request_id, const char* input_name, int imgsz) {
    ...
    td.shape[2] = imgsz;
    td.shape[3] = imgsz;
    td.data_size = (size_t)CHANNELS * imgsz * imgsz * sizeof(float);
...
static bool valid_output(const Tensors* out) {
    return out && out->num_tensors >= 1 && out->tensors[0].data != nullptr;
}
...
int main(int argc, char** argv) {
    ...
    if (!path0) {
        std::cerr << "Usage: " << argv[0]
                  << " <model.onnx> [model2.onnx] [--input-name NAME] [--imgsz N] [--no-validate]"
                  << std::endl;
        return 1;
    }
    if (!path1) path1 = path0;

    std::cout << "=== Multi-model async inference test (Hailo) ===" << std::endl;
    ...
    std::thread consumer([&]() {
        for (int i = 0; i < total; ++i) {
            ...
            if (!no_validate && !valid_output(output)) shape_errors++;

            int id = output->id;
            bool id_ok = (model_id == 0 && id >= 0 && id < N_REQUESTS) ||
    ...
    ASSERT(shape_errors == 0, std::to_string(shape_errors.load()) + " output(s) had invalid data");
