 ...
 *
 * Usage:
 *   ./lifecycle_test [model.onnx] [--input-name NAME] [--imgsz N]
 */
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== OAAX v2 Runtime Lifecycle Tests (Hailo) ===" << std::endl;
    ...
    if (!model_path) {
        ...
        std::cout << "      Usage: " << argv[0] << " <model.onnx> [--input-name NAME] [--imgsz N]" << std::endl;
    ...
    std::cout << "\n[9] Inference round-trip works on cycle 2 (after full teardown/reinit)" << std::endl;
    {
        ...
        RuntimeStatus st = runtime_retrieve_output(&out_mid, &output, 10000);
